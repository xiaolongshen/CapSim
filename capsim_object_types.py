...
class Chemical:
    ...
    def copy(self):
        ...
        if self.soluable == 0:
            ...
            chemical.kineticchemnum = self.kineticchemnum
            chemical.kineticchemnum = self.kineticchemnum
    ...
    def get_solid_chemical(self, component_name, chemical_name, MW, nn):

        self.name           = component_name + '_' + chemical_name
        self.component_name = component_name
        self.chemical_name  = chemical_name
        self.MW             = MW
        self.kineticchemnum = nn
        self.formula        = 0
        self.temp           = 0
        self.Dw             = 0
        self.Koc            = 0
...
class MatrixComponent:
    ...
    def __init__(self, number):
        ...
        self.number    = number
        self.tort      = 'Linear--Kd specified'
        self.sorp      = 'Millington & Quirk'
    ...
    def get_q(self, C, Cmax):
        ...
        if self.kinetic == self.kinetics[0]:
            ...
            if self.isotherm == self.isotherms[2]:
                ...
                if Cmax < 0.0001: Cmax = 0.0001
                if C <= Cmax * Fmin:
                    q = freundlich(Cmax * Fmin, self.Kf, self.N)[0] * C
                else:
...
class Layer:
    ...
    def bio_copy(self):
        ...
        layer.nsolidchemicals           = self.nsolidchemicals
        layer.chemicals                 = self.chemicals
        layer.solidchemicals            = self.solidchemicals
        layer.components                = self.components
        layer.component_list            = self.component_list
        layer.init_components           = self.init_components
        layer.init_component_list       = self.init_component_list
        layer.lreactions                = self.lreactions
        layer.nlreactions               = self.nlreactions
        layer.chemical_list             = self.chemical_list
        layer.solidchemical_list        = self.solidchemical_list
        layer.nlowersolidchemicals      = self.nlowersolidchemicals
        layer.lowersolidchemicalnums    = self.lowersolidchemicalnums
        layer.lowersolidchemicals       = self.lowersolidchemicals
        try:
            layer.solidcomponents           = self.solidcomponents
            layer.nsolidcomponents          = self.nsolidcomponents
        except: pass
...
class BC:
    ...
    def topboundarywidget(self, frame, row, column, topBCtype):

        if topBCtype == 'Fixed Concentration':
            ...
            if self.tau == 0:   tau_1 = 'Infinity'
            else:               tau_1 = round_to_n(1./self.tau,3)
            self.kentry   = Entry(frame, width = 8, textvariable = self.k, justify = 'center')
            self.taulabel = Label(frame, width = 8, text = tau_1, justify = 'center')
            self.kentry.grid(   row = row    , column = column,  padx = 2, pady = 1)
            self.taulabel.grid( row = row + 1, column = column,  padx = 2, pady = 1)
...
class System:
    ...
    def get_systemproperties(self, systemproperties):
        ...
        if self.bio <> 'None' and (self.layers[0].name == 'Deposition' or self.layers[0].h < self.hbio or self.bio == 'Depth-dependent') and self.Dbiop> 0 and self.nlayers > 1:     self.biomix = 1
        else:                                                                                                                                                                        self.biomix = 0
    ...
    def get_layerconditions(self, layerconditions):
        ...
        for chemical in self.chemicals:
            layerconditions.BCs[chemical.name].get_BC()
            for layer in self.layers:
                if layer.name == 'Deposition':
                    if self.topBCtype == 'Mass transfer':               layerconditions.ICs[layer.name][chemical.name].uniform.set(layerconditions.BCs[chemical.name].Cw)
                    if self.topBCtype == 'Fixed Concentration':         layerconditions.ICs[layer.name][chemical.name].uniform.set(layerconditions.BCs[chemical.name].Co)
                    if self.topBCtype == 'Finite mixed water column':   layerconditions.ICs[layer.name][chemical.name].uniform.set(layerconditions.BCs[chemical.name].Cw)
                layerconditions.ICs[layer.name][chemical.name].get_IC()

        self.BCs        = layerconditions.BCs.copy()
        self.ICs        = layerconditions.ICs.copy()
    ...
    def get_solveroptions(self, solveroptions):
        ...
        self.massoption   = solveroptions.massoption.get()
    ...
    def get_inputoptions(self, solveroptions):
        ...
        self.cpsmfilename    = solveroptions.inputname.get()
        self.batchfileoption = solveroptions.batchfileoption.get()
        self.batchfilename   = solveroptions.batchfilename.get()
