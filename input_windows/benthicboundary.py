...
class TauEstimator:
    ...
    def __init__(self, master, version, fonttype, timeunit, lengthunit, concunit, chemicals, taucoefs, BCs, U):
        ...
        for chemical in self.chemicals:
            ...
            self.Cinits.append(BCs[chemical.name].Cw.get())
    ...
    def make_widgets(self):
        ...
        self.Cinitlabel    = Label(self.frame, text = 'Initial water concentrations:')
        self.Cinitwidget   = Button(self.frame, text = 'Change concentration', justify = 'center', command = self.editCinit)
        ...
        self.Cinitlabel.grid(     row = 9, column = 1, sticky = 'E',  pady = 2)
        self.Cinitwidget.grid(     row = 9, column = 2, sticky = 'WE')

        self.Decaylabel.grid(   row = 10, column = 1, sticky = 'E',  pady = 2)
        self.Decaywidget.grid(  row = 10, column = 2, sticky = 'WE')

        self.Evaplabel.grid(    row = 11,column = 1, sticky = 'E',  pady = 2)
        self.Evapwidget.grid(   row = 11,column = 2, sticky = 'WE')
    ...
    def editCinit(self, event = None):


        if self.top is None:

            self.top = CapSimWindow(master = self.master, buttons = 2)
            self.top.make_window(CinitEditor(self.top, self.version, self.fonttype, self.concunit, self.chemicals, self.Cinits))
            self.top.tk.mainloop()

            if self.top.window.cancelflag == 0:
                for n in range(len(self.chemicals)):
                    if self.chemicals[n].soluable == 1:
                        self.Cinits[n] = self.top.window.Cinits[n].get()
    ...
    def calculate(self, event = None):
        ...
        if self.Q.get() - self.Qevap.get() + self.V.get()/self.h.get()*self.U < 0:
...
class CinitEditor:
    ...
    def __init__(self, master, version, fonttype, concunit, chemicals, Cinits):
        """Constructor method."""
        ...
        self.Cinits = []
        for n in range(len(self.chemicals)):
            self.Cinits.append(DoubleVar(value = Cinits[n]))
    ...
    def make_widgets(self):
        ...
        self.klabel      = Label(self.frame,  width = 25,  text = 'Initial water concentration (' + self.concunit+')')
