...
class LayerConditions:
    ...
    def click_tau(self, event = None):
        ...
        if self.top is None:
            ...
            self.top = CapSimWindow(master = self.master, buttons = 2)
            self.top.make_window(TauEstimator(self.top, self.version, self.fonttype, self.timeunit, self.lengthunit, self.concunit, self.chemicals, self.taucoefs,  self.BCs, self.U))
            self.top.tk.mainloop()
