...
class MainMenu:
    ...
    def make_widgets(self):
        ...
        self.outputbutton = Button(self.frame, text = 'Load existing output file', width = 20, command = self.load_output)
        self.graphbutton= Button(self.frame, text = 'Plot existing results',width = 20, command = self.graph)
        ...
        self.outputbutton.grid(row = 6, pady = 1)
        self.graphbutton.grid(row = 7, pady = 1)
        self.batchbutton.grid(row = 8, pady = 1)
        self.databutton.grid(row = 9, pady = 1)
        self.solidbutton.grid(row = 10, pady = 1)
        self.exitbutton.grid(row = 11, pady = 1)
        self.blank1.grid(row = 12)
        self.blank2.grid(row = 13)
    ...
    def load(self, event = None):
        ...
        self.option.set(1)
        filename = tkfd.askopenfilename(initialdir = Filepath + r'/input_cpsm_files', filetypes = [('CPSM files', '*.cpsm')])
        if filename == '': self.option.set(6)
        self.filename.set(filename)
        self.frame.quit()

    def load_output(self, event = None):
        """Get input file choice."""

        self.option.set(7)
        filename = tkfd.askopenfilename(initialdir = Filepath + r'/output', filetypes = [('Output files', '*.csv')])
        if filename == '': self.option.set(6)
        self.filename.set(filename)
        self.frame.quit()
