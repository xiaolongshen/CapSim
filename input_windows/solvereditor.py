...
from capsim_functions    import consolidation, tidal, round_to_n
...
class SolverEditor:
    ...
    def __init__(self, master, system, ptype, ptotal, tvariable, delt, delz, players, depgrid):
        """Constructor method."""
        ...
        if system.adv == 'Period oscillation': self.U = self.U + system.Vtidal
        ...
        self.depgrid        = IntVar(value = depgrid)                               #minimumu time steps
        ...
        for i in range(system.nlayers):
            ...
            for n in range(system.nchemicals):
                self.D[i].append(self.layers[i].get_D(self.chemicals[n].Dw * self.diff_factor, self.U, matrix.e) )
                if self.layers[i].number < 2 and system.bio == 1:
                    self.D[i][n] = self.D[i][n] + self.Dbiopw
                    for component in matrix.components:
                        self.D[i][n] = self.D[i][n] + self.Dbiop * component.fraction * component.rho * self.sorptions[component.name][chemical.name].get_K(component, self.Cmax[chemical.name], self.Cmax[chemical.name])
        ...
        if self.layers[0].number == 0:
            self.deplayersolveroptions = [LayerSolverOption(self.layers[0], self.players[0], self.D[0], self.U, self.R[0], self.timeunit)]
            self.dep = 1
        else:
            self.dep = 0
        ...
        for i in range(self.dep, self.nlayers):
            layer = self.layers[i]
            self.layersolveroptions.append(LayerSolverOption(layer, self.players[i], self.D[i], self.U, self.R[i]))
    ...
    def updatesolverparameters(self):
        ...
        if self.ptype.get() == self.ptypes[0]:
            ...
            for i in range(self.dep, self.nlayers):
                ...
                if delz_temp > 1:  self.layersolveroptions[i-self.dep].delz = (round(delz_temp,3))
                else:
                    j = 2
                    while delz_temp/100 < 0.1**j:
                        j = j + 1
                    self.layersolveroptions[i-self.dep].delz = (round(delz_temp,j))
                ...
                if self.biomix == 1 and delt_temp > self.layersolveroptions[i-self.dep].delz**2 / 2. / self.Dbiop:
                    delt_temp = self.layersolveroptions[i-self.dep].delz**2 / 2. / self.Dbiop
            ...
            if self.dep == 1:
                ...
                if delz > 1:  self.deplayersolveroptions[0].delz = (round(delz,3))
                else:
                    j = 2
                    while delz/100 < 0.1**j:
                        j = j + 1
                    self.deplayersolveroptions[0].delz = (round(delz,j))
            ...
            if self.dep == 1:
                ...
                if delz_temp > 1:  self.deplayersolveroptions[0].delz = (round(delz_temp,3))
                else:
                    j = 2
                    while delz_temp/100 < 0.1**j:
                        j = j + 1
                    self.deplayersolveroptions[0].delz = (round(delz_temp,j))
...
class LayerSolverOption:
    ...
    def __init__(self, layer, player, D, U, R, timeunit = 'yr'):
        ...
        self.Pe      = round(U * self.delz / min(D), 2)
        delt_temp    = min([R[n] * self.delz**2 / 2. / D[n] for n in range(len(D))])
