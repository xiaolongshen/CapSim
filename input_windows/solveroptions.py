...
class SolverOptions:
    ...
    def __init__(self, master, system, editflag = 0):
        ...
        self.timeoptions    = ['Implicit', 'Crank-Nicolson']
        self.averageoptions = ['Instaneous', 'Average']
        self.massoptions    = ['Track', 'Do not track']

        self.duration       = DoubleVar(value = 100)
        self.outputsteps    = IntVar(value = 100)
        self.averageoption  = StringVar(value = 'Instaneous')
        self.massoption     = StringVar(value = 'Track')
        self.discrete       = StringVar(value = self.pvariables[0])
        self.ptotal         = IntVar(value = 0)
        self.delt           = DoubleVar(value =  0)
        ...
        self.nlerror        = DoubleVar(value = 0.001)                                                          #iteration error tolerance
        ...
        try:
            self.massoption.set(system.massoption)
    ...
    def make_widgets(self):
        ...
        self.massoptionlabel  = Label(self.frame, text = 'Mass balance options:')
        self.massoptionmenu   = OptionMenu(self.frame, self.massoption, *self.massoptions)
        self.massoptionmenu.config(width = 16)
        ...
        self.massoptionlabel.grid(row =  row, column = 0, sticky = 'E', padx = 2, pady = 4)
        self.massoptionmenu.grid( row =  row, column = 1, sticky = 'WE')
        row = row + 1
        ...
        if self.discrete.get() == self.pvariables[1]:
            ...
            if event != None:
                if self.top is None:
                    ...
                    self.top.make_window(SolverEditor(self.top, self.system, self.ptype, self.ptotal.get(), self.tvariable, self.delt.get(), self.delz, self.players, self.depgrid.get()))
            ...
            self.solvereditor = SolverEditor(self.master, self.system, self.ptype, self.ptotal.get(), self.tvariable, self.duration.get()/100, self.delz, self.players, self.depgrid.get())
